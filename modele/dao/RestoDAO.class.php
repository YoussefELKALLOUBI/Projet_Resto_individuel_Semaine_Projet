...
class RestoDAO {
    ...
    public static function deleteResto(int $idR): ?bool {
        $ok = true;
        try {
            $requete = "delete FROM resto WHERE idR = :idR";
            $stmt = Bdd::getConnexion()->prepare($requete);
            $stmt->bindParam(':idR', $idR, PDO::PARAM_INT);
            $ok = $stmt->execute();
        } catch (PDOException $e) {
            throw new Exception("Erreur dans la méthode " . get_called_class() . "::deleteResto : <br/>" . $e->getMessage());
        }
        return $ok;
    }
    public static function suppressionParId(int $idR): bool {
        $resultat = false;
        try {
            $requete = "DELETE FROM aimer WHERE idR  = ?;"
                    . "DELETE FROM critiquer WHERE idR  = ?;"
                    . "DELETE FROM photo WHERE idR  = ?;"
                    . "DELETE FROM proposer WHERE idR  = ?;"
                    . "DELETE FROM resto WHERE idR  = ?;";


        $stmt = Bdd::getConnexion()->prepare($requete);
        $stmt->bindParam(1, $idR, PDO::PARAM_INT);
        $stmt->bindParam(2, $idR, PDO::PARAM_INT);
        $stmt->bindParam(3, $idR, PDO::PARAM_INT);
        $stmt->bindParam(4, $idR, PDO::PARAM_INT);
        $stmt->bindParam(5, $idR, PDO::PARAM_INT);
    ...
    public static function addResto(String $nomR, int $numAdrR, String $voieAdrR, int $cpR, String $villeR, double $latitudeDegR, double $longitudeDegR, String $descR, String $horairesR) {
        $ok = true;
        try {
    ...
    //modifier (nom, adresse, descriptif, horaire, photo, type de cuisine (rajouter un éventuellement))
    public static function getAllByTypeConsom(string $extraitNomR): array {
        $lesObjets = array();
        try {
            $requete = "SELECT * FROM resto WHERE nomR LIKE :nomR";
            $stmt = Bdd::getConnexion()->prepare($requete);
    ...
    //modifcation des informations d'un restaurant
    public static function update(Resto $unResto): bool {
        $ok = false;
        try {
            $requete = "UPDATE resto SET nomR = :nomR, numAdrR = :numAdrR, voieAdrR = :voieAdrR, cpR = :cpR, villeR = :villeR, horairesR = :horairesR, descR= :descriptionR  WHERE idR = :idR";
            $stmt = Bdd::getConnexion()->prepare($requete);
            $stmt->bindValue(':nomR', $unResto->getNomR(), PDO::PARAM_STR);
            $stmt->bindValue(':numAdrR', $unResto->getNumAdr(), PDO::PARAM_STR);
            $stmt->bindValue(':voieAdrR', $unResto->getVoieAdr(), PDO::PARAM_STR);
            $stmt->bindValue(':cpR', $unResto->getCpR(), PDO::PARAM_STR);
            $stmt->bindValue(':villeR', $unResto->getVilleR(), PDO::PARAM_STR);
            $stmt->bindValue(':horairesR', $unResto->getHorairesR(), PDO::PARAM_STR);
            $stmt->bindValue(':descriptionR', $unResto->getDescR(), PDO::PARAM_STR);
