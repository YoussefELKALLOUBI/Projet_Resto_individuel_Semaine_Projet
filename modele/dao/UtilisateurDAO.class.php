...
//        $stmt->bindValue(':mdpU', $mdpUCrypt, PDO::PARAM_STR);
    ...
    public static function deleteUser(int $idU): bool {
        $ok = false;
        try {
            ...
            throw new Exception("Erreur dans la méthode " . get_called_class() . "::deleteUser : <br/>" . $e->getMessage());
        }
        return $ok;
    }
        ...
        try {
            ...
            if ($ok) {
                ...
                while ($enreg = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    //Instancier un nouveau user et l'ajouter à la liste
                    $lesUsers[] = self::enregistrementVersObjet($enreg);
                }
            }
        } catch (PDOException $e) {
    ...
    private static function enregistrementVersObjet(array $enreg): Utilisateur {
        ...
        $leUtilisateur = new Utilisateur($idU, $enreg['mailU'], $enreg['mdpU'], $enreg['pseudoU'], $enreg['adminU']);
